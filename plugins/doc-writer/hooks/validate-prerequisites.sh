...
# Validates that the environment is ready for doc-writer operations.
# Run as a PreToolUse hook before Bash tool executions.
#
# Credentials
#   GITHUB_TOKEN / GH_TOKEN    — used by gh CLI and git push for GitHub remotes
#   AZURE_DEVOPS_TOKEN         — used by git push and REST API for Azure DevOps remotes
...
# GitHub CLI — used for PR comments and follow-up PRs on github.com remotes
...
if ! git rev-parse --is-inside-work-tree > /dev/null 2>&1; then
    echo '{"decision": "block", "reason": "Not inside a git repository. doc-writer requires a git project."}'
    exit 0
fi

# For commit operations — require git identity
...
if echo "$COMMAND" | grep -qE "^git push"; then
    ...
    if echo "$REMOTE_URL" | grep -qE "(dev\.azure\.com|visualstudio\.com)"; then
        ...
        # GitHub or generic HTTPS remote — use GITHUB_TOKEN or GH_TOKEN
